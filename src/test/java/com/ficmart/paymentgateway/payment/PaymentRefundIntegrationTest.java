...
import com.ficmart.paymentgateway.payment.infrastructure.bank.dto.BankRefundResponse;
...
class PaymentRefundIntegrationTest {
    ...
    void setUpBankResponses() {
        ...
        when(mockBankClient.refund(any(), anyString()))
                .thenAnswer(invocation -> {
                    var request = invocation.getArgument(
                            0,
                            BankRefundRequest.class
                    );

                    var response = new BankRefundResponse();
    ...
    private void capturePayment(String paymentReference)
            throws Exception {
    ...
    private String createCapturedPayment() throws Exception {
        var paymentReference = authorizePayment();

        capturePayment(paymentReference);

        return paymentReference;
    }

    private ResultActions refundPayment(
            String paymentReference,
            String idempotencyKey
    ) throws Exception {
        var request = new RefundRequest(paymentReference);
    ...
    @Test
    void shouldRefundCapturedPaymentSuccessfully()
            throws Exception {

        var paymentReference = createCapturedPayment();
        var idempotencyKey =
                "refund-" + UUID.randomUUID();

        refundPayment(paymentReference, idempotencyKey)
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.status")
                                .value("REFUNDED")
    ...
    @Test
    void shouldReturnStoredResponseWhenRefundRequestIsRetried()
            throws Exception {

        var paymentReference = createCapturedPayment();
        var idempotencyKey =
                "refund-retry-" + UUID.randomUUID();
    ...
    @Test
    void shouldReturnConflictWhenRefundIdempotencyKeyIsReusedForDifferentPayment()
            throws Exception {

        var firstPaymentReference = createCapturedPayment();
        var secondPaymentReference = createCapturedPayment();

        var idempotencyKey =
                "refund-conflict-" + UUID.randomUUID();

        refundPayment(firstPaymentReference, idempotencyKey)
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.status")
                                .value("REFUNDED")
        ...
        refundPayment(secondPaymentReference, idempotencyKey)
